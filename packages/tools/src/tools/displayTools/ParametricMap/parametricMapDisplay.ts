...
import addParametricMapToElement from './addParametricMapToElement';
import removeParametricMapFromElement from './removeParametricMapFromElement';
import { isVolumeSegmentation } from '../../segmentation/strategies/utils/stackVolumeCheck';
import { polySeg } from '../../../stateManagement/segmentation';

const { VIEWPORT_PRESETS } = csConstants;
const MAX_NUMBER_COLORS = 255;
const parametricMapConfigCache = new Map();
...
function removeSegmentationRepresentation(
  ...
  _removeParametricMapFromToolGroupViewports(
    toolGroupId,
...
async function render(
  ...
  console.log('>>>>> ParametricMapDisplay :: render');
  const {
    colorLUTIndex,
    active,
    segmentationId,
  ...
  if (isVolumeSegmentation(labelmapData, viewport)) {
    ...
    //   await _addParametricMapToViewport(
    //     viewport,
    //     labelmapData,
    //     segmentationRepresentationUID
    //   );
  ...
  _applyPreset(viewport, actorEntry);

  // _setParametricMapColorAndOpacity(
  //   viewport.id,
  //   actorEntry,
  //   cfun,
  //   ofun,
...
function _applyPreset(
  ...
  // TODO: Fix RGB transfer function range
  actorEntry.actor.getProperty().getRGBTransferFunction(0).setRange(0, 1);

  // TODO: Fix for stack viewport
  (<Types.IVolumeViewport>viewport).setProperties({ colormap }, volumeId);
}

function _setParametricMapColorAndOpacity(
  viewportId: string,
  actorEntry: Types.ActorEntry,
  ...
  const { outlineWidth, renderOutline, outlineOpacity } =
    _getParametricMapConfig(
...
function _getParametricMapConfig(
  toolGroupLabelmapConfig: LabelmapConfig,
...
function _needsTransferFunctionUpdate(
  ...
  const oldConfig = parametricMapConfigCache.get(cacheUID);

  if (!oldConfig) {
    parametricMapConfigCache.set(cacheUID, {
      fillAlpha,
      renderFill,
      renderOutline,
      outlineWidth,
...
function _removeParametricMapFromToolGroupViewports(
  toolGroupId: string,
  segmentationRepresentationUID: string
): void {
  ...
  for (const viewportInfo of viewportsInfo) {
    ...
    removeParametricMapFromElement(
...
async function _addParametricMapToViewport(
  viewport: Types.IVolumeViewport | Types.IStackViewport,
  labelmapData: LabelmapSegmentationData,
  segmentationRepresentationUID
): Promise<void> {
  await addParametricMapToElement(
    viewport.element,
    labelmapData,
    segmentationRepresentationUID
  );
