...
abstract class Spline implements ISpline {
   ...
   */
  public deleteControlPointAt(index: number): boolean {
   ...
   */
  public getClosestControlPoint(point: Point2): ClosestControlPoint {
   ...
   * @returns
   */
  public getClosestPoint(point: Point2): ClosestSplinePoint {
    this._update();
    ...
    for (let i = 0; i < curveSegmentsDistInfo.length; i++) {
      ...
      for (let j = 0; j < lineSegments.length; j++) {
        ...
        const { point: lineSegPoint, distanceSquared: lineSegDistSquared } =
          math.line.getClosestPointDistanceSquaredInfo(
    ...
    const curveSegmentLengthToPoint =
      ...
      math.line.dist(minDistLineSegment.points.start, closestPoint);
   ...
   */
  public getClosestControlPointLinesPoint(point: Point2): ClosestPoint {
  ...
  private _getCurveSegmmentsDistanceSquaredInfo(
    ...
    for (let i = 0; i < curveSegments.length; i++) {
      ...
      const distanceSquared = math.aabb.getPointAABBDistanceSquared(
        curveSegment.aabb,
        point
      );
  ...
  private _getCurveSegmmentsWithinRange(
    ...
    if (
      math.aabb.getPointAABBDistanceSquared(this.aabb, point) > maxDistSquared
    ) {
      return [];
    }
